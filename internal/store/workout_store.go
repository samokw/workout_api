...
// Think an I handling data is so i probably need a struct to manage the data
type Workout struct {
	ID              int            `json:"id"`
...
// Our application layer will only ever communicate using these functions so even if we change the underlying database we would only need to update the functions not the application layer
